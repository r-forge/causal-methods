...
### Selections saved in `slseModels` objects.
...
We see that the model keeps the original knots, so we lost nothing by
replacing `model` with the new one. The other argument is `BLSE` which
contains information about the selection. If we want to compare `BLSE`
with `FLSE`, we can call the `selSLSE` once more and replace `model`
with the new one:
...
used for the `PVT` criterion and the selections are saved in the
elements `PVT`, `JAIC` and `JBIC`. The `J` in front of AIC and BIC
means that the selection is based on the joint AIC and BIC, as opposed
to a selection model by model. There is no `J` in front of `PVT`
...
the `cslseModel` object, because it is assumed that AIC and BIC are
obtained jointly for causal SLSE models. However, if we select the
knots of one of the SLSE model separately, we will see the difference
when we print the model. For example, the following replace the SLSE
model of the treated with the SLSE model selected by AIC (separately):
...
model for the treated has many different selection elements:
...
vectors for the BLSE with AIC for the treated:
...
`selCrit` is set to `"PVT"` that the selection is done for the chosen
criterion only.

...
accessed using the operator $. The element `knots` is an object of
class `cslseKnots` and it contains all knots for the treated and
nontreated. Its print method shows all knots for both groups. To print
the knots of a specific group, we just specify which one we want to
see. For example, to print the knots of the treated we proceed as
follows:

```{r}
model1$knots$treated
```
...
```{r}
selK <- list(nontreated=list(ed = NULL, 'age:ed' = NULL, re75 = c(1000,5000,10000)))
model <- cslseModel(re78 ~ treat | ~ age * ed + re75 + married, data = nsw,
                    knots = selK)
model$knots$nontreated
```
...
- `selKnots`: As for the `knots` argument of `cslseModel`, it is a
  ...
  `list(treated=list(1L,c(3L, 5L)))`. By default it is set to `NULL`
  and all the knots from the model are used.

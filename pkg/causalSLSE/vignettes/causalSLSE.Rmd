...
`slseModel` with its own print method, which will be presented later.
...
treated and nontreated. The term `sample size` refers either to the
number of observations in the treated or nontreated group.
...
the list of knots for the nontreated and treated groups,
respectively. For example, in our case the initial knots for the
treated are:
...
selecting a subset of `Default` using SLSE, which stands for
Semiparametric Least Squares Estimator (@giurcanu16).
...
The user has control over the selection of knots through the arguments
...
`estModel` which has three arguments, but two of them are mainly used
...
We illustrate the use of `estModel` with a simple model containing 2
...
The object of class `slseFit` returned by `estModel` has its own print
...
SLSE (BSLSE) and the forward SLSE (FSLSE) methods. For each method, we
...
The function returns a model of class `slseModel` with the optimal
...
For example, the BSLSE-AIC method has removed all knots from `re75`
...
the following example, we see BSLSE as selection method and BIC as
...
Since the function `selSLSE` function returns a new model, we can
...
method returns an object of class `causalslse`. We see above what its
...
The object `causalslse` inherits from the class `slseFit`, so we can
...
`causalslse`, which is a required method for creating Latex tables
...
## The `causalSLSE` method for `slseModel` objects

When applied directly to `slseModel` objects, the `causalSLSE` method
...
`causalslse`. The arguments are the same as the method for `slseFit`
...
- **selType**: This is the selection method. We have the choice
  between "SLSE" (the default), "FSLSE" and "BSLSE". The SLSE method
...
- **selCrit**: This is the criterion used by the selection method when
  `selType` is set to "FSLSE" or "BSLSE". The default is "AIC".
  
- **pvalT**: This is a function that returns the p-value threshold. We
  explained this argument when we presented the `selSLSE` function.
...
function and the `causalSLSE` method for `slseModel` objects. It
...
Note that this method calls `setModel`, `selSLSE`, `estModel` and the
...
linear. We can compare the different fits (we ignore the FSLSE because
...
can compare the SLSE, BSLSE-AIC and BSLSE-BIC.
...
The following shows the fit of BSLSE-AIC with the true $f_1(x)$ and
...
$10^7$). We can compare the SLSE, FSLSE with AIC and FSLSE with BIC.
...
the FSLSE-BIC method fits the curves.
...
how these curves are approximated by FSLSE-AIC.
...
compare the SLSE, FSLSE-AIC and FSLSE-BIC.

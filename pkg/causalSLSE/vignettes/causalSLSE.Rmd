...
models. We could use it to generate knots for any regression model
estimated by SLSE. But, estimating such models is not yet implemented
in the package. If we want to create a set of knots for a specific
group, we can restrict the sample using the `subset` function. For
example, we can create the initial knots of `model1` for the treated
and nontreated as follows:

```{r}
k1.mod1 <- slseKnots(form = ~ age + re75 + ed + married,
                  data = subset(nsw, treat==1))
k0.mod1 <- slseKnots(form = ~ age + re75 + ed + married,
                  data = subset(nsw, treat==0))
```

Since we only need a list of confounders to create the knots, the
argument `form` is a regular formula and only the right-hand side is
used by the function. The function returns an object of class
`slseKnots` and its `print` method produces a nice display separating
confounders with and without knots. For example, the following are the
starting knots for the treated:
...
of `married` because it is a binary variable:
...
function. For this function, we need the same type of formula used
above to create a model. For example, the above two sets of knots can
be created as follows:

```{r}
k.mod1 <- cslseKnots(form = re78 ~ treat | ~ age + re75 + ed + married,
                     data = nsw)
```

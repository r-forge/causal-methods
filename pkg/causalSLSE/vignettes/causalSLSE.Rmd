...
to the formula. The function `cslseModel` creates an object of class
`cslseModel` with its own print method, which will be presented later.
...
model <- cslseModel(re78 ~ treat | ~ age * ed + re75 * ed + married * ed, data = nsw,
...
```{r}
estSLSE(selSLSE(model1, selType = "FSLSE", selCrit = "BIC"))
```
...
library(texreg)
c1 <- causalSLSE(fit1)
fit2 <- estSLSE(selSLSE(model1, selType="BSLSE"))
fit3 <- estSLSE(selSLSE(model1, selType="FSLSE"))
c2 <- causalSLSE(fit2)
c3 <- causalSLSE(fit3)

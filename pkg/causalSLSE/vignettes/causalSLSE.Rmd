...
\frac{1}{n_1}\sum_{i=1}^n U_{i, k}Z_i$ and $\bar{U}_{k, 0} =
\frac{1}{n_0}\sum_{i=1}^n U_{i, k}(1-Z_i)$, for $k=0,1$, where $n_0$
and $n_1$ are the sample size in the nontreated and treated groups and
$n=n_1+n_0$. 
...
To see where the function `vcov.` is used, note that the causal
effects can be expressed as a linear combination of the vector of
least squares estimates. Let $\hat{\theta}=\tp{\{\hat\beta_0,
\hat\beta_1, \tp{\hat\psi_0}, \tp{\hat\psi_1}\}}$. Then, the causal
effect estimates can be written as
$\tp{\hat{D}_{\mathrm{c}}}\hat{\theta}$ for c=ACE, ACT or ACN, with
$\hat{D}_{\mathrm{ACE}}=\tp{\{-1, 1, \tp{-\bar{U}_0},
\tp{\bar{U}_1}\}}$, $\hat{D}_{\mathrm{ACT}}=\tp{\{-1, 1,
\tp{-\bar{U}_{0,1}}, \tp{\bar{U}_{1,1}}\}}$ and
$\hat{D}_{\mathrm{ACN}}=\tp{\{-1, 1, \tp{-\bar{U}_{0,0}},
\tp{\bar{U}_{1,0}}\}}$. Since $\hat{D}_\mathrm{c}$ is random, we need
a first order Taylor expansion to derive the variance of the causal
estimates. For example, we can show that the variance of
...
$\hat\Sigma_{k,l}=\widehat{\cov(\bar{U}_k, \bar{U}_l)}$, $\hat\Sigma_{k,
\hat\theta}=\tp{\hat\Sigma_{\hat\theta, k}}=\widehat{\cov(\bar{U}_k,
\hat\theta)}$, for $k,l=0,1$, and $\hat\Sigma_{\hat\theta}$ is the
estimated variance of $\hat\theta$ computed by `vcov.`. Simulations
show that using `vcovHC` with `type="HC3"` produces the most accurate
estimate of the variance of ACE, ACT and ACN in small and large
samples, which justifies using this specification as default.
...
plot(c2, "X", main="Y vs X using BLSE-BIC")
curve(1 - 2 * x, -3, 3, col="darkgreen", lty = 4, lwd = 3, add = TRUE)
curve(1 + x + x^2, -3, 3, col = "darkorange", lty = 4, lwd = 3, add = TRUE)
legend("bottomleft", c("True-treated", "True-nontreated"),
       col = c("darkgreen", "darkorange"), lty = 4, lwd = 3, bty = 'n')
plot(c3, "X", main="Y vs X using FLSE-BIC")
curve(1 - 2 * x, -3, 3, col="darkgreen", lty = 4, lwd = 3, add = TRUE)
curve(1 + x + x^2, -3, 3, col = "darkorange", lty = 4, lwd = 3, add = TRUE)
legend("bottomleft", c("True-treated", "True-nontreated"),
       col = c("darkgreen", "darkorange"), lty = 4, lwd = 3, bty = 'n')
...
plot(c2, "X", legendPos = "right", cex = .8, main="Y vs X using BLSE-AIC")
...
```{r, out.width='50%', fig.show='hold'}
plot(c2, "X1", legendPos = "right", cex = .8, main="Y vs X1 using FLSE-AIC")
...
```{r, out.width='50%', fig.show='hold'}
plot(c2, "X2", legendPos = "right", cex = .8, main="Y vs X2 using FLSE-AIC")

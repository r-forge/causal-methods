...
selected using the procedure described in the previous section. The
function selects the knots using the `slseKnots` and stores them in
the object under `knots`. We can print them using the \$ operator as
follows and compare them with the ones obtained in the previous
section:
...
treatment group. The object also contains names of the variable
representing the treatment indicator ($Z$) and what values of this
indicator is associated with the treated and the nontreated. The
function has the same arguments as `slseModel`, plus the argument
`groupInd` that specifies which value of $Z$ is associated with the
treated and which one is associated with the nontreated. The default
is `groupInd=c(treated = 1, nontreated = 0)`. It is possible to have
other values or even characters as indicator, but the names must be
...
Since `model1` is a list of `slseModel` objects, stored as `treated`
and `nontreated`, we can access the knots of a specific group as
follows
...
print the knots of a specific gropup using the command
`print(model1$treated, which="selKnots")'. To print the list of knots
for both groups, we can proceed as follows:
...
described in Section \ref{sssec:slseModel}. In the following we create
the treatment indicator variable `treat` equal to "treat" for the
treated and "notreat" for the nontreated.

```{r}
simDat4$treat <- ifelse(simDat4$Z==1, "treat", "notreat")
```

We can create a `cslseModel` object by specifying the value associated
...
is simply estimating the `slseModel` objects separately. We illustrate
the use of `estSLSE` with a simple model containing 2 confounders and
one knot per variable. 
...
We see that the coefficients are separated by group. The coefficients
for the treated correspond to $\{\hat\beta_1, \hat\psi_1\}$ and the
coefficients for the nontreated correspond to $\{\hat\beta_0,
\hat\psi_0\}$. Note that the `cslseFit` object returned by the method
is a list of `slseFit`, which is the object returned by `estSLSE` when
it is applied to `slseModel`. We can access the estimated SLSE model
...
- **addToLegend**: An optional character string to add to the legend
  next to "treated" and "nontreated". Note that a legend is added when
  applied to `slseFit` objects, so this argument has no effect in this
  case.
...
- **FUN**: A function to determine how the other confounders are
  ...
  each group separately (obviously since the plot is applied to each
  group separately).

...
The general causal semiparametric additive regression model is 
...
example, a two treatment model is like a one treatment model with one
more SLSE model. 
...
@lalonde86. The purpose of this dataset is to measure the causal effect
...
create a SLSE model using `re78` as dependent variance and the same
...
the print function shows the number of knots for each covariate inside
the parentheses next to its name. In this example, we see that the
initial number of knots for `age`, `re75` and `ed` are respectively 7,
4 and 5. The function selects the knots using the `slseKnots` and
stores them in the object under `knots`. We can print them using the
...
-  A numeric vector: The vector cannot contain missing or duplicated
   values and must be strictly inside the range of the variable.
...
In that case, the knots must be defined in the same order of variables
implied by the formula. For example, if we want to set an automatic
selection for `age`, no knots for `ed` and the knots $\{1000, 5000,
10000\}$ for `re75`, we proceed as follows. Note that setting the
...
**Case 3: A named list of length strictly less than the number of confounders.**
...
previous example, the knots are set manually only `age`. By default,
...
Note also that `slseModel` deals with interaction terms as any other
...
`ed:black` has many zeros and the quantiles equal to 0 are removed.
...
`slseModel` methods. Since this paper is about causal-SLSE, we choose
to present these methods through the `cslseModel` object.

## The causal-SLSE model (`cslseModel`) \label{ssec:cslseModel}

The function `cslseModel` returns an object of class `cslseModel` (or
causal-SLSE model). It is a list of `slseModel` objects, one for each
treatment group.  The function has the same arguments as `slseModel`,
plus the argument `groupInd` that specifies which value of $Z$ is
associated with the treated and which one is associated with the
nontreated. The default is `groupInd=c(treated = 1, nontreated =
0)`. It is possible to have other values or even characters as
indicator, but the names must be treated and nontreated. We will allow
more names in future version of the package once the multiple
...
like the `slseModel` object, but the information is provided by group
...
Alternatively, we can use the print method for `slseModel` objects and
print the knots of a specific group using the command
...
If some values in the treatment indicator variable differ from the
...
`knots`. The procedure is the same, but we need to specified the name
of the group associated with the knots. If `knots` is set to `NULL`,
the number of knots it set to 0 for all confounders and all groups. If
we only want the number of knots to be 0 for one group, we need to
specify the group. For example, the number of knots is set to 0 for
the treated only in the following:
...
The 3 cases presented in Section \ref{sssec:slseManKnots} are the
same, but it applies group by group. If a group is missing from the
argument `knots`, the knots of the missing group are set
automatically. For example, if we want to set the knots as in Case 1,
but only for the nontreated and let `cslseModel` choose them for the
treated, we would proceed as follows:
...
- `selKnots`: It is a list of one or two elements, one for each
  ...
  knot for the first confounder and knots 3 and 5 for the second for
  the treated and all knots for the nontreated, we set `selKnots` to
...
`summary` method. The only arguments of `summary` is the `cslseFit`
...
`cslseFit` objects simply apply the method for `slseFit` objects on
...
- **fixedCov**: Optional named lists of fixed values for some or all
	...
	covariates for both groups, `fixedCov` is just a named list with
...
effect estimators has only been proved for the last two in
...
- **vcovType**: The type of LSE covariance matrix used to compute the
  ...
  slows down the process, especially for FLSE, and simulations show
  that the choice of HC has very little effect on the
...
change the selection method and/or the criterion, the selection is
saved in the 'slseModel` object. Let's consider the following model:
...
regressions are needs for the `PVT` criterion, but we need one
...
replacing `model` by the new one. The other argument is `BLSE` which
...
model of the treated by the SLSE model selected by AIC (separately):
...
vectors for the BLSE with AIC:

```{r}
model$treated$selections$BLSE$JAIC
```

For example, we see that the knots 1 and 3 of `age` is selected by
...
`cslseModel` objects and work very similarly. The arguments are
`selType`, `selCrit` and `selKnots`. The latter is used to select the
knots manually and we will not cover this option. The First two are as
in the `selSLSE` method. The purpose of `update` is to replace the
current knots by a selection saved in the model. If the selection we
want is not in the model, `update` will return an error message. For
example, the object `model` from the previous section has all
selections saved, but the current is a mixture of FLSE-AIC and
BLSE-AIC (Sep.).
...
we want to recover the initial model, we simply set the argument
`selType` to `"None"`. In the following, we see that the selection
method is back to its initial selection:
...
- **vcov.**: An alternative function used to compute the covariance
  ...
  default, `vcovHC` is used with `type="HC3"`. Simulations show that
  using `vcovHC` with `type="HC3"` produces the most accurate estimate
  of the variance of ACE, ACT and ACN in small and large samples.
...
effect estimates. We can extract any causal estimate and its standard
error by using the \$ operator followed by the type of causal
estimate. For example, the following is the ACE:
...
which includes the `Default` and `User Based` methods. The arguments
of the `extract` methods, which control what is printed and can be
modified through the `texreg` function, are:
...
`cslse`. The arguments are the same as the method for `cslseFit`
...
standard errors. In particular, this approach does not offer the more
efficient option of computing all selections once and save them in the
model as explained at the end of the previous section.
...
- **slseModel**: The object is created by the function `slseModel` and
  the exported registered methods are `print`, `estSLSE` (estimate the
  regression model), `selSLSE` (optimal selection of knots), `update`
  (select knots from saved selections) and `causalSLSE` (to compute
  the causal effects). There are two non-exported methods: `pvalSLSE`
...
- **cslseModel**: The object is created by the function `cslseModel`
  ...
  the regression model), `selSLSE` (optimal selection of knots),
  `update` (select knots from saved selections) and `causalSLSE` (to
  compute the causal effects). There are two non-exported methods:
  `pvalSLSE` (used to compute the p-values) and `model.matrix` (to
  extract the matrix of confounders).
  
- **slseFit** and **cslseFit**: The objects are created by the method
  `estSLSE` (the former when apply to `slseModel` object and the
  ...
  outcome as a function of one confounder), `summary` (to give more
...
- **cslse**: The object is created by any `causalSLSE` method. It
  inherits from `cslseFit` object. The methods that are common through

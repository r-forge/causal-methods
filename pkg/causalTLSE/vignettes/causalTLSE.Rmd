...
abstract: "This vignette explains how to use the causalTLSE package to estimate causal effects using the semiparametric thresholding least squares methods developed by @tlse23."
vignette: >
...
This document presents the `causalTLSE` package describing the
...
q)\in\reals^{p_k}$, with $p_k=\sum_{l=1}^q p_{l,k}$. In this paper, we
propose a data-driven method for selecting the vectors of basis
functions $u_0(X)$ and $u_1(X)$. Note that we allow the number of
basis functions ($p_{l,k}$) to differ across confounders and groups.
...
of $X$, for any categorical variable with two levels, the number of
knots must be equal to zero. In this case, $u(x)=x$. For general
ordinal variables, the number of knots cannot exceed the number of
levels minus two. The following illustrates the local linear basis
function when the number of knots is equal to 3:

```{r, echo=FALSE, fig.align='center', out.width='50%', fig.width=10, fig.height=6}
knots <- c(2,5,7.5)
maxX <- 11.5
minX <- -2
plot(NULL, xlim=c(minX,maxX), ylim=c(minX,knots[2]), axes=FALSE, ylab="u(x)", xlab="x")
...
recommended since we already model nonlinearities using basis
functions. This is presented to give an example of what can be added
to the formula. The function `setModel` creates an object of class
`tlseModel` with its own print method, which will be presented later.
...
variable based on the following procedure applied separately for the
treated and nontreated. The term `sample size` refers to the number of
observations in the treated or nontreated group.
...
the list of knots for the nontreated and treated groups. For example, in
...
package). This can be seen as follows:

```{r}
quantile(nsw[nsw$treat==1,'re75'], c(.2,.4,.6,.8), type=1)
```
   
By printing the object, we see a description of the model. It includes
the list of variables with a positive number of knots and with no
knots.
...
selecting a subset of `Default` using TLSE, which stands for
Thresholding Least Squares Estimator (@giurcanu16).
...
categorical variable `X2` with 3 character-type levels, an ordinal
variable `X3` with 3 levels, and a binary variable `X4`. The levels
for `X2` are {"first","second","third"} and for `X3` the levels are
{1,2,3}.
...
We have the control over the selection of knots through the arguments
`knots0` and `knots1`. When the arguments are missing (the default),
all knots are set automatically as described above. One way to set the
number of knots to 0 for all variables in a given group is to set the
argument to `NULL`. For example, the number of knots is equal to 0 for
all variables of the treated group using the following command:
...
the knots are provided manually by the user. Also, the print method
shows the lists of covariates by group only when they differ, which is
the case here. The other option is to provide a list of knots. For
each element, we have three options:
...
1. An unnamed list of length equal to the number of covariates. In
  ...
  `married`. In the following, the argument `knots=TRUE` is added to
  the `print` method to only print the knots.
...
2. A named list of length equal to the number of covariates. In that
  case, the order of the list of variables does not matter. The
	  `setModel` function will automatically reorder the variables to
  ...
  following, we change the order of knots to show that the order does
  not matter.
...
Note also that `setModel` deals with interaction terms as any other
...
as the basis functions corresponding to the set of knots. The
...
the knots of the model). The function that estimates the model is
`estModel`. The function has three arguments, but two of them are
mainly used internally by other functions. We present them in case
they are needed. The arguments are:

- `model`: A model created by the function `setModel`.

- `w0` and `w1`: lists of integers to select knots for the nontreated and
  treated respectively. For example, suppose we have 2
...
The object of class `tlseFit` returned by `estModel` has its own print
...
TLSE (BTLSE) and the forward TLSE (FTLSE) methods. For each method, we
...
For example, the BTLSE-AIC method has removed all knots from `re75`
for the treated and kept two knots for the nontreated. The
...
least squares estimates of the regression parameters. Then, the TLSE
...
- **selCrit**: This is the criterion used by the selection method when
  `selType` is set to "FTLSE" or "BTLSE". The default is "AIC".
...
create a model. The arguments are the same as of the `setModel`
function and the `causalTLSE` method for `tlseModel` objects. It
...
can compare the SLSE, BTLSE-AIC and BTLSE-BIC.
...
compare the SLSE, FTLSE-AIC and FTLSE-BIC.
